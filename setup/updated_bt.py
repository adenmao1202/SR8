...
def execute_short_strategy(open_np, close_np, vwap_np, open_time_np, initial_cap, fee_rate, 
    ...
    """
    idx_len = len(open_np)  # open_np will have different values every day 
    equity = np.full(idx_len, initial_cap)
    position_size = 0.0
    trades = 0
    cap = initial_cap  # Initial capital stays fixed until we exit the position
    ...
    # Average of the first five K-bars to decide whether to enter the trade
    first_5_open_avg = np.mean(open_np[:5][open_np[:5] != 0])  # Ignore 0 values in the first 5 bars

    # Check if the average of the first 5 K-bars exceeds the threshold (shorting logic)
    ...
    for i in range(idx_len):
        ...
        # Skip if the open price is 0 (no trade at this time)
        if open_np[i] == 0 or close_np[i] == 0:
            equity[i] = cap  # Keep equity unchanged if no trade occurs
            continue

        # Enter short position if conditions are met
        if current_time >= entry_time_min and i >= 5 and not has_traded_today:
            entry_price = open_np[i]
            position_size = initial_cap / entry_price  # Shorting with full capital
        ...
        if has_traded_today and position_size > 0:
            ...
            # Correct calculation of equity for a short position (profit when prices drop)
            equity[i] = cap + (position_size * (entry_price - current_price))

            # Exit conditions: VWAP breach
            if current_price > vwap_np[i]:
                exit_price = current_price
                transaction_fee = position_size * exit_price * fee_rate
                cap += position_size * (entry_price - exit_price) - transaction_fee  # Update capital after closing
                position_size = 0
                equity[i] = cap
                continue

            # Stop-loss condition (triggered if the price rises by stop_loss_pct)
            if current_price >= entry_price * (1 + stop_loss_pct):
                exit_price = current_price
                transaction_fee = position_size * exit_price * fee_rate
                cap += position_size * (entry_price - exit_price) - transaction_fee  # Update capital after stop loss
                position_size = 0
                equity[i] = cap
                continue

            # Partial exits at defined times
            while (partial_exit_index < len(partial_exit_times) and
                ...
                equity[i] = cap + (position_size * (entry_price - current_price))  # Update equity for remaining position
            ...
            if i == idx_len - 1 and position_size > 0:
                # Exit remaining position at the end of the day
                exit_price = current_price
                transaction_fee = position_size * exit_price * fee_rate
